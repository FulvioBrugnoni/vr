...
class Command(BaseCommand):
    ...
    def handle(self,*args,**kwargs):
        ...
        with open(self.BASEDIR+'algoritmorecruiter.csv') as csv_file:
            ...
            for row in csv_reader:
                ...
                if counter ==0:
                    ...
                    continue
                try:
                    algoritmo = Algoritmo.objects.get(id =row[0])
                except ObjectDoesNotExist:
                    self.stderr.write(f'Elemento {row[0]} non presente nel db')
                    continue
                try:
                    recruiter = Recruiter.objects.get(id =row[1])
                except ObjectDoesNotExist:
                    self.stderr.write(f'Elemento {row[1]} non presente nel db')
                    continue
                try:
                    azienda = Azienda.objects.get(id =row[2])
                except ObjectDoesNotExist:
                    self.stderr.write(f'Elemento {row[1]} non presente nel db')
                    continue
                try:
                    importazione = AlgoritmoRecruiter(algoritmo=algoritmo, recruiter=recruiter,azienda=azienda, stato =row[3])
                    importazione.save()
